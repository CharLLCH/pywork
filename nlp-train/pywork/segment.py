...
def test(pin):
    ...
    for x in temp_list:
        print (str(x)+' '+str(pro_dict[x][2])).decode('gb2312').encode('utf-8')
    
#dir_word,dir_pin=Creat_words('lexicon.txt')
#print '正在分词和构建词表……'.decode('gb2312').encode('utf-8')
...
# pro_dict=Transform_to_word('haoren',dir_pin,dir_word,sum_word,theta)
# temp_list=sorted(pro_dict.keys(),key=lambda x:pro_dict[x][2],reverse=True)
# print temp_list
f_word=open('__f_word','rb')
#pickle.dump(dir_word,f_word)
dir_word=pickle.load(f_word)
f_word.close()
f_pin=open('__f_pin','rb')
#pickle.dump(dir_pin,f_pin)
dir_pin=pickle.load(f_pin)
f_pin.close()
f_sum=open('__f_sum','rb')
